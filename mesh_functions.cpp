...
{
    long double a, b;
    long double d1 = ((t[0][0] - px) * (t[0][0] - px) +
                 (t[0][1] - py) * (t[0][1] - py) +
                 (t[0][2] - pz) * (t[0][2] - pz));
    long double d2 = ((t[1][0] - px) * (t[1][0] - px) +
                 (t[1][1] - py) * (t[1][1] - py) +
                 (t[1][2] - pz) * (t[1][2] - pz));
    if(d2 < d1) d1 = d2;
    ...
    const long double    x1 = t[0][0] - px, y1 = t[0][1] - py, z1 = t[0][2] - pz,
                    x2 = t[1][0] - t[0][0], y2 = t[1][1] - t[0][1], z2 = t[1][2] - t[0][2],
                    x3 = t[2][0] - t[0][0], y3 = t[2][1] - t[0][1], z3 = t[2][2] - t[0][2];

    const long double    a_inter = -(x3 * y1 - x1 * y3) / (x3 * y2 - x2 * y3),
    ...
    const long double d = x3 * x3 * (y2 * y2 + z2 * z2) + (y3 * z2 - y2 * z3) * (y3 * z2 - y2 * z3) -
...
long double distance_to_mesh(triangle *mesh, int x, int y, int z, size_t mesh_size)
{
    ...
    long double d_min = distance_to_triangle(mesh[0], x, y, z, intersected), d;
